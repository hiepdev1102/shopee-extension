...
function listen(socket) {
    ...
    socket.on('join-room', (room) => {
        ...
        Object.keys(io.sockets.adapter.rooms).forEach(_room => {
            if (_room === room) {
                check = false;
                if (Object.keys(io.sockets.adapter.rooms[_room].sockets).length <= constraint.ROOM_SIZE) {
                    //add currentRoom;
                    ...
                    socket.emit('join-room', { status: true, link: link, room: room, full: false  });
                }
                else{
                    socket.emit('join-room', { status: false, full: true });
                }
                return false;
            }
        });
